...
const getAllOrder = (data, paginationOption, filter) =>
  __awaiter(void 0, void 0, void 0, function* () {
    // pagination
    ...
    const { searchTerm } = filter,
      filtersData = __rest(filter, ['searchTerm'])
    const andCondition = []
    // if (searchTerm) {
    //   andCondition.push(
    //     {
    ...
    if (data.role === 'user') {
      const result = yield order_model_1.Order.find({ user: data.id }).populate(
        'user',
      )
      return result
    } else {
...
const getSingleOrder = id =>
  __awaiter(void 0, void 0, void 0, function* () {
    const result = yield order_model_1.Order.findById({ _id: id }).populate(
      'user',
    )
    return result
  })
